...
- `GlobalExceptionHandler` (`@RestControllerAdvice`) extends
  `ResponseEntityExceptionHandler` so framework client errors (malformed JSON, UUID
  mismatch, 405, 415) stay 4xx. Domain mapping: `ResourceNotFoundException` → 404,
  `MethodArgumentNotValidException` → 400 with field-level `details`, catch-all
  `Exception` → 500. Every handler returns a consistent `ErrorResponse` body.
