 ...
 *
 * <p>JNI allows Java code to call (and be called by) native applications and libraries written in
 * C, C++, or assembly. Common use cases: hardware access, legacy library integration,
 * performance-critical native code.
 *
 * <h3>Workflow</h3>
 *
 * <ol>
 *   <li>Declare {@code native} methods in Java
 *   <li>Generate C header: {@code javac -h output/native src/.../JniExample.java}
 *   <li>Implement the C functions matching the generated signatures
 *   <li>Compile to shared library: {@code gcc -shared -o libnative.so -I$JAVA_HOME/include ...}
 *   <li>Load at runtime via {@link System#loadLibrary(String)}
 * </ol>
 *
...
public class JniExample {
  ...
  static {
    try {
      ...
      System.err.println(
          "Native library not found — JNI methods will throw. "
              + "Build the C library first. See README for instructions.");
    }
  }

  /**
   * Native method: adds two integers in C. Maps to: {@code
   * Java_com_example_template_systems_JniExample_nativeAdd}
   *
  ...
  /**
   * Pure-Java fallback — use when native library is unavailable. This pattern lets you ship a Java
   * fallback alongside native acceleration.
   *
   * @param a first operand
   * @param b second operand
   * @return the sum {@code a + b}
