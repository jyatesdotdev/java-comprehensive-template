 ...
 *
 * <ul>
 *   <li>No GC pauses for large buffers
 *   <li>Direct I/O without copying (zero-copy networking, memory-mapped files)
 *   <li>Predictable memory layout for interop with native code
 * </ul>
 *
...
public final class OffHeapMemoryExample {
  ...
  /**
   * Allocates a direct ByteBuffer (off-heap) and performs typed read/write. Direct buffers are
   * deallocated when the ByteBuffer is GC'd (via internal Cleaner).
   *
  ...
  public static ByteBuffer directBufferDemo(int capacityBytes) {
    ByteBuffer buf = ByteBuffer.allocateDirect(capacityBytes).order(ByteOrder.nativeOrder());
  ...
  /**
   * A simple off-heap array of doubles with deterministic cleanup via {@link Cleaner}. Demonstrates
   * the recommended Java 9+ pattern for releasing native resources.
   */
  ...
  public static class OffHeapDoubleArray implements AutoCloseable {
    ...
    public OffHeapDoubleArray(int length) {
      this.length = length;
      this.buffer = ByteBuffer.allocateDirect(length * Double.BYTES).order(ByteOrder.nativeOrder());
