 ...
 *
 * <p>Events are scheduled on a priority queue ordered by simulation time. The engine processes
 * events one at a time in chronological order.
 *
 * <h3>Usage — M/M/1 Queue</h3>
 *
 * <pre>{@code
 * var sim = new DiscreteEventSimulation();
 * var queue = new MM1Queue(sim, 0.8, 1.0); // arrival rate λ=0.8 < service rate μ=1.0 (stable, ρ=0.8)
...
public final class DiscreteEventSimulation {
   ...
   *
   * <ul>
   *   <li>Arrivals follow a Poisson process (inter-arrival times are exponential)
   *   <li>Service times are exponentially distributed
   *   <li>There is one server
   * </ul>
   */
  public static final class MM1Queue {
    ...
    private void handleArrival() {
      ...
      if (serverBusy) {
        ...
        sim.scheduleDelay(serviceTime, "departure", () -> handleDeparture(now));
      }
    }
    ...
    private void handleDeparture(double arrivalTime) {
      ...
      if (!waitingQueue.isEmpty()) {
        ...
        busyTime -= now;
        double serviceTime = exponential(serviceRate);
        sim.scheduleDelay(serviceTime, "departure", () -> handleDeparture(waitingSince));
      } else {
        serverBusy = false;
      }
    }
    ...
    public void printStats() {
      ...
      System.out.printf(
          "  Server util (ρ):   %.4f  (theoretical: %.4f)%n", busyTime() / simTime, rho());
      System.out.printf(
          "  Avg system time:   %.4f  (theoretical: %.4f)%n",
          departures > 0 ? totalSystemTime / departures : 0, 1.0 / (serviceRate - arrivalRate));
    ...
    /**
     * Returns cumulative server busy time up to the current clock.
     *
     * <p>Busy intervals are accumulated as {@code departureTime - serviceStartTime} (subtract on
     * start, add on departure). If a service is still in progress, the raw accumulator is short by
     * the open interval's start time, so this accessor closes it at the current clock.
     *
     * @return busy time in simulation units
     */
    public double busyTime() {
      return serverBusy ? busyTime + sim.clock() : busyTime;
    }
  }
