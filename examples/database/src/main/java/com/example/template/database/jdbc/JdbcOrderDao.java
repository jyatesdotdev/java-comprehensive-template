 ...
 *
 * <ul>
 *   <li>Complex SQL that doesn't map well to JPA/JPQL
 *   <li>Bulk operations for performance
 *   <li>Stored procedure calls
 *   <li>Fine-grained control over result set mapping
 * </ul>
 */
@Repository
public class JdbcOrderDao {
  ...
  public JdbcOrderDao(JdbcTemplate jdbc) {
    ...
    this.orderInsert =
        new SimpleJdbcInsert(jdbc).withTableName("orders").usingGeneratedKeyColumns("id");
  }
  ...
  private static final RowMapper<OrderSummary> ORDER_SUMMARY_MAPPER =
      (rs, rowNum) ->
          new OrderSummary(
              rs.getLong("id"),
  ...
  public int countByStatus(OrderStatus status) {
    Integer count =
        jdbc.queryForObject(
            "SELECT COUNT(*) FROM orders WHERE status = ?", Integer.class, status.name());
  ...
  public long insertOrder(String customerName) {
    Number key =
        orderInsert.executeAndReturnKey(
            Map.of(
                "customer_name",
                customerName,
                "status",
