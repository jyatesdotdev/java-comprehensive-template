 ...
 *
 * <ul>
 *   <li>{@link ConcurrentHashMap} — compute, merge, search, reduce
 *   <li>{@link CopyOnWriteArrayList} — read-heavy, write-rare scenarios
 *   <li>{@link BlockingQueue} — producer-consumer pattern
 *   <li>Atomic variables and accumulators
 *   <li>Synchronization primitives: CountDownLatch, Semaphore, Phaser
 *   <li>Lock strategies: ReentrantReadWriteLock, StampedLock
 * </ul>
 */
...
public final class ConcurrentCollectionsExamples {
  ...
  public static ConcurrentHashMap<String, Long> wordCount(List<String> words) {
    ...
    words.parallelStream().forEach(w -> counts.merge(w, 1L, Long::sum));
    return counts;
  }
  ...
  /**
   * Simple producer-consumer using a bounded {@link java.util.concurrent.ArrayBlockingQueue}. The
   * poison pill pattern signals the consumer to stop.
   */
  ...
  /**
   * {@link LongAdder} outperforms {@link AtomicLong} under high contention because it spreads
   * updates across cells and sums lazily.
   *
  ...
  /**
   * A point class demonstrating {@link StampedLock} optimistic read pattern. Optimistic reads avoid
   * acquiring a lock, falling back to a read lock only if the data was modified during the read.
   */
