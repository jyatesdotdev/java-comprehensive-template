 ...
 *
 * <ul>
 *   <li>I/O-bound tasks: HTTP calls, database queries, file I/O
 *   <li>High-concurrency servers (thread-per-request model)
 * </ul>
 *
 * <h3>When NOT to use virtual threads</h3>
 *
 * <ul>
 *   <li>CPU-bound computation — use parallel streams or ForkJoinPool instead
 *   <li>Tasks holding native resources or using {@code synchronized} heavily (pinning)
 * </ul>
 */
@SuppressWarnings({
...
public final class VirtualThreadExamples {
  ...
  public static Thread startVirtualThread(Runnable task) {
    return Thread.ofVirtual().name("vt-worker").start(task);
  }

  /**
   * ExecutorService backed by virtual threads — the recommended approach for structured concurrency
   * in server applications.
   *
   * <p>Each submitted task runs on its own virtual thread. The executor creates a new virtual
   * thread per task (no pooling needed — they're cheap).
   *
  ...
  public static List<String> fetchAllUrls(List<String> urls) throws Exception {
    try (var executor = Executors.newVirtualThreadPerTaskExecutor()) {
      List<Future<String>> futures =
          urls.stream().map(url -> executor.submit(() -> simulateHttpGet(url))).toList();

      // Collect results — blocks on each future but the virtual threads
      // unmount from carrier threads during I/O waits
      return futures.stream().map(VirtualThreadExamples::getUnchecked).toList();
    }
  }

  /**
   * Demonstrates massive concurrency — 100k virtual threads running concurrently. This would be
   ...
   *
   * <p>This is pseudo-code showing the pattern. Actual usage requires: {@code --enable-preview} and
   * import of {@code java.util.concurrent.StructuredTaskScope}.
   *
   * <pre>{@code
