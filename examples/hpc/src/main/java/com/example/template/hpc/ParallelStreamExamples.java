 ...
 *
 * <ul>
 *   <li>Use parallel streams for CPU-bound work on large datasets
 *   <li>Avoid parallel streams for I/O-bound or small collections
 *   <li>Use custom ForkJoinPool to isolate parallelism from the common pool
 *   <li>Ensure operations are stateless, non-interfering, and associative
 * </ul>
 */
@SuppressWarnings({
  "PMD.LooseCoupling",
...
public final class ParallelStreamExamples {
  ...
  public static long sumOfSquares(int limit) {
    return IntStream.rangeClosed(1, limit).parallel().mapToLong(i -> (long) i * i).sum();
  }

  /**
   * Run a parallel stream on a custom ForkJoinPool to avoid starving the common pool. This is
   * critical in server applications where the common pool is shared.
   *
   ...
   */
  public static <T> Map<String, List<T>> groupByClassParallel(List<T> items, int parallelism)
      throws Exception {
  ...
  /**
   * Thread-safe collection with parallel stream using {@code groupingByConcurrent}. More efficient
   * than {@code groupingBy} in parallel because it uses a shared ConcurrentMap instead of merging
   * per-thread maps.
   *
  ...
  public static ConcurrentMap<Integer, List<Integer>> groupByModulo(
    ...
    return numbers.parallelStream().collect(Collectors.groupingByConcurrent(n -> n % modulo));
  }

  /**
   * Demonstrates ordered vs unordered parallel processing. {@code unordered()} can improve
   * performance by removing ordering constraints.
   *
  ...
  /**
   * Unordered variant of {@link #firstNPrimes(int)} — allows {@code limit()} to short-circuit
   * faster at the cost of non-deterministic ordering.
   *
   * @param count the number of primes to find
