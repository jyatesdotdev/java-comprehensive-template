 ...
 *
 * <ul>
 *   <li>Async execution with custom executors
 *   <li>Chaining: thenApply, thenCompose, thenCombine
 *   <li>Fan-out/fan-in: allOf, anyOf
 *   <li>Error handling: exceptionally, handle, whenComplete
 *   <li>Timeouts (Java 9+)
 * </ul>
 */
@SuppressWarnings({
...
public final class CompletableFutureExamples {
  ...
  /**
   * Chain dependent async operations. fetchUser → fetchOrders → computeTotal
   *
  ...
  public static CompletableFuture<Double> computeUserOrderTotal(
    ...
    return CompletableFuture.supplyAsync(() -> fetchUser(userId), executor)
        ...
        .thenApply(orders -> orders.stream().mapToDouble(Double::doubleValue).sum());
  }
  ...
  public static CompletableFuture<String> fetchDashboard(ExecutorService executor) {
    var profileFuture = CompletableFuture.supplyAsync(() -> "UserProfile", executor);
    var statsFuture = CompletableFuture.supplyAsync(() -> "Stats{orders=42}", executor);
    var notificationsFuture = CompletableFuture.supplyAsync(() -> "Notifications[3]", executor);

    return CompletableFuture.allOf(profileFuture, statsFuture, notificationsFuture)
        .thenApply(
            v ->
                String.join(
                    " | ", profileFuture.join(), statsFuture.join(), notificationsFuture.join()));
  }
  ...
  public static CompletableFuture<String> combineResults(ExecutorService executor) {
    ...
    return priceFuture.thenCombine(
        stockFuture,
        (price, stock) -> String.format(Locale.ROOT, "Price: $%.2f, Stock: %d", price, stock));
  }

  /**
   * Error handling with recovery. The exceptionally handler provides a fallback value when the
   * upstream computation fails.
   *
  ...
  public static CompletableFuture<String> withFallback(
    ...
    return CompletableFuture.supplyAsync(riskyOp, executor).exceptionally(ex -> fallback);
  }
   ...
   * @param executor the executor to run the operation on
   * @return a future that times out with {@link java.util.concurrent.TimeoutException} if not
   *     completed in time
   */
