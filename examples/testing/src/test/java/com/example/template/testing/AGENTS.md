...
- The suffix IS the routing: `*Test` → surefire (`mvn test`); `*IT` → failsafe
  (`mvn verify -P integration-tests`). This module's pom excludes `**/*IT.java` from
  surefire — name files accordingly or they run in the wrong phase (or not at all).
