 ...
 *
 * <p>Demonstrates the core Spring Batch 5 patterns:
 *
 * <ul>
 *   <li>Chunk-oriented processing with configurable chunk size
 *   <li>{@link FlatFileItemReader} for delimited file input
 *   <li>{@link ItemProcessor} for business transformation
 *   <li>{@link JsonFileItemWriter} for structured output
 *   <li>Automatic retry/skip policies (see step configuration)
 * </ul>
 *
 * <h3>Usage</h3>
 *
...
public class CsvToJsonBatchConfig {
  ...
  public record OutputRecord(
      String id, String name, double amountUsd, String category, String tier) {}
  ...
  /**
   * Transforms an {@link InputRecord} into an {@link OutputRecord} by parsing the amount, filtering
   * non-positive values, and computing a tier classification.
   *
  ...
  /**
   * Defines the chunk-oriented step that reads, processes, and writes records. Configured with
   * fault tolerance to skip up to 10 {@link NumberFormatException}s.
   *
  ...
  public Job csvToJsonJob(JobRepository jobRepository, Step csvToJsonStep) {
    return new JobBuilder("csvToJsonJob", jobRepository).start(csvToJsonStep).build();
  }
}
