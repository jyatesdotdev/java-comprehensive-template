 ...
 *
 * <p>Spark runs in local mode here for demonstration. In production, submit jobs via {@code
 * spark-submit} with cluster manager configuration.
 *
 * <h3>Running</h3>
 *
 * <pre>
...
public final class SparkEtlExample {
   ...
   *
   * <p>Pattern: Extract (parallelize) → Transform (flatMap/mapToPair/reduceByKey) → Collect
   *
  ...
  public static List<scala.Tuple2<String, Integer>> wordCountRdd(List<String> lines) {
    ...
    try (JavaSparkContext sc = new JavaSparkContext(conf)) {
      ...
      return rdd.flatMap(
              line -> Arrays.asList(line.toLowerCase(Locale.ROOT).split("\\W+")).iterator())
   ...
   */
  public static Dataset<SalesRecord> typedDatasetExample(
      SparkSession spark, List<SalesRecord> data) {
    ...
    return ds.filter((FilterFunction<SalesRecord>) r -> r.revenue() > 100.0)
        ...
        .mapGroups(
            (MapGroupsFunction<String, SalesRecord, SalesRecord>)
                (region, iter) -> {
                  double total = 0;
                  int count = 0;
                  while (iter.hasNext()) {
